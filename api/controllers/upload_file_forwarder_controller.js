...
const ForwarderReport = require('../models/forwarder_report');
...
router.all('/upload-forwarder', function (req, res, next) {
  ...
  router.all('/get-upload-forwarder', function (req, res, next) {
    res.setHeader("Access-Control-Allow-Origin", "*");
    res.setHeader("Access-Control-Allow-Methods", "GET,HEAD,OPTIONS,POST,PUT");
    res.setHeader("Access-Control-Allow-Headers", "Access-Control-Allow-Headers, Access-Control-Allow-Origin, Origin, Accept, X-Requested-With, Content-Type, Access-Control-Request-Method, Access-Control-Request-Headers");
    next();
...
router.post('/upload-forwarder', upload.single('file'), async (req, res) => {
    ...
    if (req.file) {
        ...
        if (fileEncryped == null) {
            return res.json('the user you entered does not have a public key ');
        }
...
router.post('/get-upload-forwarder', async (req, res) => {
    ...
    const allReports = await ForwarderReport.findAll({
